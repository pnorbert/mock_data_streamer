...
def create_listener(bind_host, ports=None):
    return create_tcp_listener(bind_host, ports, backlog=len(VARIABLE_PAIRS))
...
def accept_connections(listener, session_id, public_key, allowed_networks):
    by_variables = {}
    try:
        for _ in VARIABLE_PAIRS:
            connection, address = accept_from_allowed_network(
                listener, allowed_networks
            )
            try:
